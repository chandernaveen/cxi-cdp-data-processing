...
import refined_zone.hub.identity.model.IdentityType.{CombinationCard, Email, ParbrinkCustomerId, Phone}
...
class PosIdentityProcessorIntegrationTest extends BaseSparkBatchJobTest with BeforeAndAfterEach {
    ...
    val destTable = generateUniqueTableName("integration_test_parbrink_pos_identities_processor")

    test("Write Parbrink identities to intermediate table") {
        ...
        val identities_1 = Seq(
            ...
            ("hash003", ParbrinkCustomerId.value, "1", Map.empty[String, String])
        ).toDF(CxiIdentityId, Type, Weight, Metadata)

        // when
        // write identities_1 first time
        PosIdentityProcessor.writeIdentities(identities_1, destTable, feedDateString, run_id_1)
        ...
        PosIdentityProcessor.writeIdentities(identities_2, destTable, feedDateString, run_id_1)
        ...
        PosIdentityProcessor.writeIdentities(identities_3, destTable, feedDateString, run_id_2)
