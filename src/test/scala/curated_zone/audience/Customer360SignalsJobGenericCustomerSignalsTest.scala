...
class Customer360SignalsJobGenericCustomerSignalsTest extends BaseSparkBatchJobTest {
    ...
    test("test process generic customer signals") {
        ...
        // when
        val actual = Customer360SignalsJob.processGenericCustomerSignals(
            spark,
    ...
    test("test normalize signals") {
        ...
        val signals = Seq(
            ...
            Row(
                ...
                Seq(Row("burger_affinity_boolean", "true")),
                Seq(Row("age_category_keyword", "55-59"))
            ),
            Row(
                "cust_id3",
                Seq(),
        ...
        withClue("Normalized data does not match") {
            val expected = Seq(
                ...
                (
                    ...
                    Map("delivery_preferred_boolean" -> "true"),
                    Map("burger_affinity_boolean" -> "true"),
                    Map("age_category_keyword" -> "55-59")
                ),
                (
                    "cust_id3",
                    null,
