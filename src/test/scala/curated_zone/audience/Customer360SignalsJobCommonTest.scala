    ...
    test("test process of customer & partnerlocation specific signals") {
        ...
        val partnerLocationWeeklySignals = Seq(
            ...
            (
                ...
                "some_signal",
                "loyal",
                sqlDate(signalGenerationDate.minusDays(2))
            )
        ).toDF(
            "cxi_partner_id",
            "location_id",
        ...
        // when
        val actual = Customer360SignalsJob.process(
            spark,
            DatalakeTablesConfig(
                signalUniverseTable,
                customer360Table,
                customer360SignalsTable,
        ...
        val outputToEsSchema = DataTypes.createStructType(
            Array(
                ...
                StructField(
                    "customer_identities",
                    DataTypes.createMapType(StringType, DataTypes.createArrayType(StringType, true), true),
                    true
                ),
                StructField(
                    "general_customer_signals",
                    DataTypes.createMapType(StringType, DataTypes.createMapType(StringType, StringType, true), true),
                    true
                ),
                StructField(
                    "specific_partner_location_signals",
