...
class LoyaltyTypeSignalsJobTest extends BaseSparkBatchJobTest {
    ...
    test("Read orders and identities") {
        ...
        val customer360Df = Seq(
            ...
            ("cust_id3", Map("email" -> Array("test2@domain.com")), customer360createDate, customer360createDate, false),
            ("cust_id4", Map("email" -> Array("customer4@domain.com")), customer360createDate, customer360createDate, true)
        ...
        val cxiIdentitIdsType = ArrayType(StructType(Array(
            StructField("identity_type", StringType),
            StructField("cxi_identity_id", StringType)
        )))
        val orderSummarySchema = StructType(Array(
        ...
        val orderSummary = Seq(
            Row(Seq(Row("combination-bin", "combination-bin_1")), "partner1", "loc1", BigDecimal(10.0), sqlDate("2022-02-24"), "ord1"),
            Row(Seq(Row("email", "test@test.com")), "partner1", "loc1", BigDecimal(20.0), sqlDate("2022-01-21"), "ord2"),
        ...
        withClue("Orders do not match") {
            val expected = Seq(
                Row("partner1", "loc1", BigDecimal(10.00), sqlDate("2022-02-24"), "ord1", "combination-bin:combination-bin_1"),
