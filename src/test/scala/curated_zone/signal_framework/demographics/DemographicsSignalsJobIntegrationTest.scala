...
class DemographicsSignalsJobIntegrationTest extends BaseSparkBatchJobTest with BeforeAndAfterEach {
    ...
    test("test write") {
        ...
        val firstInsertOverwrite = spark.createDataFrame(Seq(
        ...
        withClue("Saved signals do not match") {
            ...
            val expected_1 = firstInsertOverwrite.withColumn("signal_generation_date", col("signal_generation_date").cast(DateType))
        ...
        val secondInsertOverwrite = spark.createDataFrame(Seq(
        ...
        withClue("Saved signals do not match") {
            val actual_2 = spark.table(destTable)
                .select("customer_360_id", "signal_generation_date", "signal_domain", "signal_name", "signal_value")
            val expected_2 = firstInsertOverwrite.unionByName(secondInsertOverwrite)
        ...
        val thirdInsertOverwrite = spark.createDataFrame(Seq(
        ...
        withClue("Saved signals do not match") {
            val actual_3 = spark.table(destTable)
                .select("customer_360_id", "signal_generation_date", "signal_domain", "signal_name", "signal_value")
            val expected_3 = firstInsertOverwrite.unionByName(thirdInsertOverwrite)
                .withColumn("signal_generation_date", col("signal_generation_date").cast(DateType))
