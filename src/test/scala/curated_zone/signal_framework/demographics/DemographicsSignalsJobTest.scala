...
class DemographicsSignalsJobTest extends BaseSparkBatchJobTest {
    ...
    test("test readCustomer360") {
        ...
        withClue("Read data does not match." + actualData.mkString("\nActual data:\n", "\n-----------------------\n", "\n\n")) {
            val expected = Seq(
                ("uuid1", "throtle_id_1"),
                ("uuid1", "throtle_id_2"),
                ("uuid2", "throtle_id_11")
    ...
    test("test transform") {
        ...
        val tidAttschema = StructType(Array(
            StructField("throtle_id", StringType, true),
            StructField("age", IntegerType, true),
            StructField("children", BooleanType, true)
        ))
        import collection.JavaConverters._
        val refinedThrotleTidAttDf = spark.createDataFrame(Seq(
            Row("throtle_id_1", 1, true),
            Row("throtle_id_2", 2, false),
            Row("throtle_id_3", 3, null), // children signal is null -> filtered out
            Row("throtle_id_4", 4, null), // age signal for customer uuid3 contains age values [3, 4], only first value will be picked, children signal is null -> filtered out
        ...
        withClue("Transformed data does not match." + ageSignalActualData.mkString("\nActual data:\n", "\n-----------------------\n", "\n\n")) {
            val expected_1 = spark.createDataFrame(Seq(
        ...
        val childrenSignalDf = transformedDfs.find(data => data._1 == "some_other_domain" && data._2 == "children").map(_._3).get
        val childrenSignalActualData = childrenSignalDf.collect()
        withClue("Transformed data does not match." + childrenSignalActualData.mkString("\nActual data:\n", "\n-----------------------\n", "\n\n")) {
            val expected_2 = spark.createDataFrame(Seq(
