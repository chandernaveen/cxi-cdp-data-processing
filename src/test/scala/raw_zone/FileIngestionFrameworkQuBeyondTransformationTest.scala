...
class FileIngestionFrameworkQuBeyondTransformationTest extends BaseSparkBatchJobTest {
    ...
    test("test QuBeyond transformation with Checks data") {
        ...
        // when
        val actual = FileIngestionFrameworkTransformations.transformQuBeyond(spark.createDataFrame(List(landingQuBeyondCheckData)))

        // then
        val actualFieldsReturned = actual.schema.fields.map(f => f.name)
        withClue("Actual fields returned:\n" + actual.schema.treeString) {
            actualFieldsReturned shouldEqual Array("record_type", "record_value", "feed_date", "file_name", "cxi_id", "req_customer_id", "req_location_id", "req_data_type", "req_sub_data_type", "data_delta", "req_start_date", "req_end_date")
        }
        import spark.implicits._
        val actualQuBeyondRawData = actual.as[QuBeyondRawZoneModel].collectAsList()
        withClue(
            s"""Check data do not match
