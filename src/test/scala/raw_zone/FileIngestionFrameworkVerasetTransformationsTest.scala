...
class FileIngestionFrameworkVerasetTransformationsTest extends BaseSparkBatchJobTest {
    ...
    test("test veraset transformation ad_id and IP conversion") {
        // given
        val landingVerasetData = spark.createDataFrame(
            List(
        ...
        withClue("Actual fields returned:\n" + actual.schema.treeString) {
            actualFieldsReturned shouldEqual Array("id_type","advertiser_id_AAID", "advertiser_id_IDFA", "advertiser_id_UNKNOWN", "ipv_4", "ipv_6")
        }
        ...
        withClue("Actual veraset data frame data do not match") {
            ...
            ).toDF("id_type","advertiser_id_AAID", "advertiser_id_IDFA", "advertiser_id_UNKNOWN", "ipv_4", "ipv_6").collect()
