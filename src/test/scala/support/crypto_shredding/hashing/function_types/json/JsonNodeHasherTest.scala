...
class JsonNodeHasherTest extends FunSuite with Matchers {
    ...
    val piiConfig = PiiColumnsConfig(Seq(
        "record_value" -> InnerColumn.JsonPath("$.pii_column"),
        "record_value" -> InnerColumn.JsonPath("$.nested.column"),
        "record_value" -> InnerColumn.JsonPath("$.nested..recursive_column")
    ))
