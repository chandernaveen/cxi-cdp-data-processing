...
class LoyaltyTypeSignalService (
    ...
    val regularCustomerService: RegularCustomerService) extends Serializable {

    def getLoyaltyTypesForTimePeriod
        (customer360orders: DataFrame, timePeriod: CustomerMetricsTimePeriod, feedDate: String, loyaltyCfg: LoyaltyConfig): DataFrame = {
        ...
        val atRiskPerLocation = atRiskCustomerService.getAtRiskCustomersPerPartnerAndLocation(loyalPerLocation, customer360orders, endDate, loyaltyCfg)

        val atRiskAllLocations = atRiskCustomerService.getAtRiskCustomersPerPartnerForAllLocations(loyalAllLocations, customer360orders, endDate, loyaltyCfg)

        val newPerLocation = newCustomerService.getNewCustomersPerPartnerAndLocation(customer360orders, endDate, loyaltyCfg.newCustomerTimeframeDays)

        val newAllLocations = newCustomerService.getNewCustomersPerPartnerForAllLocations(customer360orders, endDate, loyaltyCfg.newCustomerTimeframeDays)
        ...
        val regularPerLocation = regularCustomerService.getRegularCustomersPerPartnerAndLocation(
            customer360orders, newLoyalAtRiskPerLocation, endDate, loyaltyCfg.regularCustomerTimeframeDays)
        val regularAllLocations = regularCustomerService.getRegularCustomersPerPartnerAllLocations(
            customer360orders, newLoyalAtRiskAllLocations, endDate, loyaltyCfg.regularCustomerTimeframeDays)
...
object LoyaltyTypeSignalService {
    def apply(
        ...
        regularCustomerService: RegularCustomerService): LoyaltyTypeSignalService = {

        new LoyaltyTypeSignalService(newCustomerService, atRiskCustomerService, loyalCustomerService, regularCustomerService)
    }
}
