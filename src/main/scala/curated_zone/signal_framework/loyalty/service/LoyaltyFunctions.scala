...
object LoyaltyFunctions {
    ...
    final val ByCustomerAndPartnerAndLocation: WindowSpec =
        Window.partitionBy("customer_360_id", "location_id", "cxi_partner_id")
    ...
    private def recencyScore(
        customer360orders: DataFrame,
        startDate: String,
        latestOrderSpec: WindowSpec,
        dayToTxnSpec: WindowSpec
    ): DataFrame = {
        customer360orders
            ...
            .withColumn(
                "recency_score",
                (col("days_to_txn") - col("min_days_to_txn")) /
                    (col("max_days_to_txn") - col("min_days_to_txn"))
            )
    }

    private[service] def recencyScorePerPartnerAndLocation(
        customer360orders: DataFrame,
        startDate: String
    ): DataFrame = {
    ...
    private[service] def recencyScorePerPartnerForAllLocations(
        customer360orders: DataFrame,
        startDate: String
    ): DataFrame = {
    ...
    private def monetaryScore(
        customer360orders: DataFrame,
        ordPalTotalSpec: WindowSpec,
        minMaxAmountSpec: WindowSpec
    ): DataFrame = {
        customer360orders
            ...
            .withColumn(
                "monetary_score",
                (col("total_amount") - col("min_amount")) /
    ...
    private def loyalIndicator(customer360orders: DataFrame, endDate: String, windowSpec: WindowSpec): DataFrame = {
        ...
        val month1 = (
            end.minusDays(monthTimeframeDays * 3 - 1).toString(DateFormat),
            end.minusDays(monthTimeframeDays * 2).toString(DateFormat)
        )
        val month2 = (
            end.minusDays(monthTimeframeDays * 2 - 1).toString(DateFormat),
            end.minusDays(monthTimeframeDays).toString(DateFormat)
        )
    ...
    private[service] def loyalIndicatorPerPartnerAndLocation(
        customer360orders: DataFrame,
        endDate: String
    ): DataFrame = {
    ...
    private[service] def loyalIndicatorPerPartnerForAllLocations(
        customer360orders: DataFrame,
        endDate: String
    ): DataFrame = {
    ...
    private def loyalty(
        ...
        customer360ordersByDateWithScores
            ...
            .withColumn(
                "rfm_score",
                (col("final_score") - col("min_final_score")) / (col("max_final_score") - col("min_final_score"))
            )
            .na
            .fill(1.0)
    ...
    private[service] def loyaltyPerPartnerAndLocation(
        customer360orders: DataFrame,
        rfmThreshold: Double,
        rfmScores: RfmScores
    ): DataFrame = {
        ...
        loyalty(
            customer360orders,
            Seq("customer_360_id", "location_id", "cxi_partner_id"),
            rfmThreshold,
            rfmScores,
            ByPartnerAndLocation
        )
    ...
    private[service] def loyaltyPerPartnerForAllLocations(
        customer360orders: DataFrame,
        rfmThreshold: Double,
        rfmScores: RfmScores
    ): DataFrame = {
    ...
    private[service] def getRfmScoresAndLoyaltyIndicatorPerPartnerAndLocation(
        customer360orders: DataFrame,
        startDate: String,
        endDate: String
    ): RfmScores = {
    ...
    case class RfmScores(
        recencyScore: DataFrame,
        frequencyScore: DataFrame,
        monetaryScore: DataFrame,
        loyalIndicator: DataFrame
    )
}
