...
class AtRiskCustomerService(val loyalCustomerService: LoyalCustomerService) extends Serializable {
    ...
    def getAtRiskCustomersPerPartnerAndLocation(
        loyalCustomers: DataFrame,
        customer360orders: DataFrame,
        endDate: String,
        loyaltyCfg: LoyaltyConfig
    ): DataFrame = {

        val prevLoyalEndDate = LocalDate
            .parse(endDate, DateTimeFormat.forPattern(DateFormat))
            .minusDays(loyaltyCfg.atRiskCustomerTimeframeDays - 1)
            .toString(DateFormat)

        val prevLoyal = loyalCustomerService
            .getLoyalCustomersPerPartnerAndLocation(
                customer360orders,
                prevLoyalEndDate,
        ...
        prevLoyal
            .join(currLoyal, Seq("customer_360_id", "location_id", "cxi_partner_id"), "left")
            // have nulls after the left join as customer might not be present in the 'current loyal' dataframe
            .na
            .fill(value = false, Seq("is_curr_loyal"))
    ...
    def getAtRiskCustomersPerPartnerForAllLocations(
        ...
        val prevLoyal = loyalCustomerService
            .getLoyalCustomersPerPartnerForAllLocations(
                customer360orders,
                prevLoyalEndDate,
                loyaltyCfg.loyalCustomerTimeframeDays,
                loyaltyCfg.rfmThreshold
        ...
        prevLoyal
            .join(currLoyal, Seq("customer_360_id", "cxi_partner_id"), "left")
            // have nulls after the left join as customer might not be present in the 'current loyal' dataframe
            .na
            .fill(value = false, Seq("is_curr_loyal"))
            .where(atRiskCondition)
