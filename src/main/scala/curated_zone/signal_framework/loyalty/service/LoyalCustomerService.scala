...
class LoyalCustomerService extends Serializable {
    ...
    def getLoyalCustomersPerPartnerAndLocation(
        ...
        val startDate = LocalDate
            .parse(endDate, DateTimeFormat.forPattern(DateFormat))
            .minusDays(loyalCustomerTimeframeDays - 1)
            .toString(DateFormat)
        val customer360ordersByDate = customer360orders
            .where(col("ord_date").between(startDate, endDate))

        val rfmScores = LoyaltyFunctions.getRfmScoresAndLoyaltyIndicatorPerPartnerAndLocation(
            customer360ordersByDate,
            startDate,
            endDate
        )
        LoyaltyFunctions
            .loyaltyPerPartnerAndLocation(customer360ordersByDate, rfmThreshold, rfmScores)
    ...
    def getLoyalCustomersPerPartnerForAllLocations(
        ...
        val rfmScores = LoyaltyFunctions.getRfmScoresAndLoyaltyIndicatorPerPartnerForAllLocations(
            customer360ordersByDate,
            startDate,
            endDate
        )
        LoyaltyFunctions
            .loyaltyPerPartnerForAllLocations(customer360ordersByDate, rfmThreshold, rfmScores)
            .where(col("is_loyal") === true)
            .withColumn(LoyaltyTypeSignalName, lit(CustomerLoyaltyType.Loyal.value))
            .select("customer_360_id", "location_id", "cxi_partner_id", LoyaltyTypeSignalName)
    }
