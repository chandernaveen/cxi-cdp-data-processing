...
object LoyaltyTypeSignalsJob {
    ...
    private val loyaltyTypeSignalService: LoyaltyTypeSignalService =
        LoyaltyTypeSignalService(newCustomerService, atRiskCustomerService, loyalCustomerService, regularCustomerService)
    ...
    def main(args: Array[String]): Unit = {
        ...
        val finalDf = process(
            spark, s"$curatedDb.$customer360TableName", s"$refinedHubDb.$orderSummaryTableName", feedDate, loyaltyConfig)

        write(finalDf, s"$curatedDb.$customer360partnerLocationWeeklySignalsTable", feedDate)
    }

    def process(spark: SparkSession, customer360Table: String, orderSummaryTable: String, feedDate: String, loyaltyConfig: LoyaltyConfig): DataFrame = {

        val (customer360identities, orders) = read(spark, customer360Table, orderSummaryTable)

        val customer360orders = orders.join(customer360identities, Seq("qualified_identity_id"), "inner")
        ...
        val loyaltyTypeSignals7days =
            loyaltyTypeSignalService.getLoyaltyTypesForTimePeriod(customer360orders, CustomerMetricsTimePeriod.Period7days, feedDate, loyaltyConfig)
        val loyaltyTypeSignals30days =
            loyaltyTypeSignalService.getLoyaltyTypesForTimePeriod(customer360orders, CustomerMetricsTimePeriod.Period30days, feedDate, loyaltyConfig)
        val loyaltyTypeSignals60days =
            loyaltyTypeSignalService.getLoyaltyTypesForTimePeriod(customer360orders, CustomerMetricsTimePeriod.Period60days, feedDate, loyaltyConfig)
        val loyaltyTypeSignals90days =
            loyaltyTypeSignalService.getLoyaltyTypesForTimePeriod(customer360orders, CustomerMetricsTimePeriod.Period90days, feedDate, loyaltyConfig)
    ...
    def read(spark: SparkSession, customer360Table: String, orderSummaryTable: String): (DataFrame, DataFrame) = {
        ...
        val customer360identities = spark.table(customer360Table)
            .where(col("active_flag") === true)
            .select(col("customer_360_id"), explode(col("identities")).as(Seq("identity_type", "identity_values_array")))
        ...
        val orders = spark.table(orderSummaryTable)
            .na.drop(Seq("cxi_identity_ids", "ord_date"))
