...
object Customer360SignalsJob {
    ...
    def readCustomer360(spark: SparkSession, customer360TableName: String): DataFrame = {
        spark.table(customer360TableName)
    ...
    def readSignalUniverse(spark: SparkSession, signalUniverseTableName: String): Dataset[SignalUniverse] = {
        ...
        broadcast(spark.table(signalUniverseTableName)
    ...
    def readPartnerLocationWeeklySignals(spark: SparkSession, partnerLocationWeeklySignalsTableName: String, feedDate: String): DataFrame = {
        val feedDateCol = to_date(lit(feedDate), "yyyy-MM-dd")
        val daysInAWeek = 7
        val excludeCurrentDay = 1

        val partnerLocationWeeklyTableWithLastWeekData = spark.table(partnerLocationWeeklySignalsTableName)
            .where(col("signal_generation_date") between(date_sub(feedDateCol, daysInAWeek), date_sub(feedDateCol, excludeCurrentDay)))
        ...
        val partnerLocationWeeklySignalsDf = partnerLocationWeeklyTableWithLastWeekData
            .join(broadcast(latestGeneratedSignals), Seq("signal_generation_date", "signal_domain", "signal_name"), "inner")
    ...
    def transformPartnerLocationSignals(spark: SparkSession,
        ...
        val recognizedSignals = partnerLocationSignalsDf
            ...
            .select(
                "domain_name", "signal_es_name",
                "es_field_parent_section_name",
                "customer_360_id", "cxi_partner_id", "location_id", "date_option", "signal_value")
        ...
        val partnerLocationSignalsGroupedByDomainsAndTimePeriods = groupPartnerLocationSignalsByDomainAndTimePeriod(signalsByDomainToMap)
        ...
        val partnerLocationSignalsEnrichedWithLocationNames = partnerLocationSignalsGroupedByDomainsAndTimePeriods
            ...
            .select("customer_360_id", "es_field_parent_section_name", "cxi_partner_id", "location_id", "location_nm", "time_periods")
    ...
    def groupPartnerLocationSignalsByDomainAndTimePeriod(signalsByDomainToMap: DataFrame): DataFrame = {
        val notDomainColumns = List("customer_360_id", "es_field_parent_section_name", "cxi_partner_id", "location_id", "date_option")
        val keyValueDomainColumnsMapping: Array[Column] =
            signalsByDomainToMap.columns.filter(c => !notDomainColumns.contains(c)).flatMap { c => Array(lit(c), col(c)) }
    ...
    def transformGenericSignals(signalsDf: DataFrame, signalUniverseDs: Dataset[SignalUniverse]): DataFrame = {
        ...
        val keyValueDomainColumnsMapping: Array[Column] =
            signalsByDomainToMap.columns.filter(c => !groupingColumns.contains(c)).flatMap { c => Array(lit(c), col(c)) }

        val esParentSectionColNames = signalUniverse.map(_.es_field_parent_section_name).toSet
        val uniqueEsSectionNames: Set[Column] = esParentSectionColNames.map(section => col("generic_customer_signals")(section).as(section))
        val allResultingColumns: Seq[Column] = col("customer_360_id") +: uniqueEsSectionNames.toList
        signalsByDomainToMap
            .withColumn("domains_as_map", map(keyValueDomainColumnsMapping:_*))
            .withColumn("domains_as_map_filtered", map_filter(col("domains_as_map"), (k, v) => v.isNotNull))
            .withColumn("generic_customer_signals_struct", struct(col("es_field_parent_section_name"), col("domains_as_map_filtered") ))
            .groupBy("customer_360_id")
            .agg(
    ...
    def extractPartnerLocationSignalDomainAsColumn(signalsWithCustomerDetails: DataFrame): DataFrame = {
        signalsWithCustomerDetails.groupBy("customer_360_id", "es_field_parent_section_name", "cxi_partner_id", "location_id", "date_option")
            .pivot("domain_name")
    ...
    def extractSignalDomainAsColumn(signalsWithCustomerDetails: DataFrame): DataFrame = {
        signalsWithCustomerDetails.groupBy("customer_360_id", "es_field_parent_section_name")
            .pivot("domain_name")
            .agg(collect_list(struct(col("signal_es_name"), col("signal_value"))))
