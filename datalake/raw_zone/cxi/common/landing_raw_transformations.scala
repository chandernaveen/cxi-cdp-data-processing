 ...
 * @function-desc - Generic Transformation (no changes)
 */
def oracleSim(df: DataFrame): DataFrame = {
  ...
  return df1.withColumn("record_type", coalesce(oracleSimColPerType.map(c => when(col(c).isNotNull, lit(c)).otherwise(lit(null))): _*))
